...
class Experiment:
    ...
    def __train_and_evaluate_model(self, model_name, X_resampled, y_resampled, replication):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_resampled, y_resampled, test_size=0.4
        )
    ...
    def __store_results(self, model_name, replication, accuracy, f1, roc_auc, precision, best_params):
        ...
        if self.logger:
            self.logger.info(
                f"Model: {model_name}, Replication: {replication + 1}, "
                f"Accuracy: {accuracy:.4f}, F1: {f1:.4f}, ROC AUC: {roc_auc:.4f}, Precision: {precision:.4f}"
            )
